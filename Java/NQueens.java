...
public class NQueens {
    ...
    public static boolean isattacked(int board[][] , int x , int y , int n) {
    ...
    public static boolean solve(int n , int board[][] , int sum) {
        if(n == 0)
            return true;
        for(int i = 0 ; i < sum; i++) {
            for(int j = 0 ; j < sum; j++) {

                // If current cell is attacked proceed to next
                if(isattacked(board , i , j , sum) == true) 
                    continue;
                
                // Else set queen there
                board[i][j] = 1;

                // If solution exist for subproblem return true
                if(solve(n - 1 , board , sum) == true)
                    return true;

                // Else backtrack by undoing previous change
                board[i][j] = 0;
            }
        }
        return false;
