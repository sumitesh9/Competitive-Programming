...
public class PassWord {
    ...
    //trie[x][y] = m represent son of xth node using yth character of alphabet , m = -1 means there is no link
    //trie[x][y] x = max number of nodes in a trie , y = alphabet set size
    static int trie[][] = new int[(int)1e5][26];
    static int size = 1;
    static int root = 0;
    static void insert(String s){
        int node = root;
        for(int i = 0; i < s.length(); i++) {
            if(trie[node][s.charAt(i) - 'A'] == -1) trie[node][s.charAt(i) - 'A'] = size++;
    ...
    public static void main(String[] args)throws IOException {
        ...
        //Insert
        for(int i = 0; i < pattern.length(); i++) {insert(pattern.substring(i , pattern.length()));}
        // Print the trie
        for(int i = 0; i < 1e5; i++) {
            for(int j = 0; j < 26; j++) {
                if(trie[i][j] != -1) {pn(i + "->" + trie[i][j] + ":" + (char)(j + 65));}
            }
        }
        pw.flush();
        pw.close();  
