...
public class day6 {
    ...
    { 
        ...
        if(b == 1)
            return a;
        if((b & 1) == 1)
            return (a % p * power((a % p * a % p) % p , (b - 1) / 2 , p) % p) % p;
    ...
    public static void main(String[] args) {
        int n = f.nextInt();
        String str = f.next();
        char x = f.next().charAt(0);
        char y = f.next().charAt(0);
        long acc = 0 , ans = 0 , count = 0;

        for(int i = 0; i < n; i++) {
            if(str.charAt(i) == y) {
                acc = ((acc % mod) + (power(2 , (long)i , mod) % mod)) % mod;
                count++;
            }
        }
        ...
        for(int i = 0; i < n; i++) {
            ...
            if(str.charAt(i) == y) {
                acc = (acc - power(2 , (long)i , mod) % mod) % mod;
            }

            if(str.charAt(i) == x) {
                long df = power(2 , (long)(i + 1) , mod);
                ans = (ans % mod + (acc % mod * power(df , mod - 2 , mod) % mod) % mod) % mod; 
            }
        }
