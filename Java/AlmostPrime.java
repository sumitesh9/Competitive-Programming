...
public class AlmostPrime {
    ...
    public static void main(String[] args)throws IOException {
        ...
        int ans = 0 , flag = 0;
        for(long i = 1; i <= n; i++) {
            HashSet<Long> fc = factors(i);
            HashSet<Long> pf = new HashSet<Long>();
            for(Long j : fc) {if(isPrime(j)) pf.add(j);} 
            if(pf.size() == 2) ans++;
        }
        pn(ans);
        pw.flush();
...
/******************************END OF MAIN PROGRAM*******************************************/
    ...
    static boolean[] sieveOfEratosthenes(int n){boolean prime[]=new boolean[n+1];for(int i=0;i<n;i++)prime[i]=true;for(int p=2;p*p<=n;p++){if(prime[p]==true){for(int i=p*p;i<=n;i+=p)prime[i]=false;}}}
