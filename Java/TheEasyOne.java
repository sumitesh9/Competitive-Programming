...
public class TheEasyOne {
    ...
    public static void main(String[] args) {
        ...
        while(t --> 0) {
            String n = f.next();
            HashMap<Character , Long> h = new HashMap<Character , Long>();
            
            for(int i = 0; i < n.length(); i++) {
                if(h.containsKey(n.charAt(i))) 
                    h.put(n.charAt(i) , h.get(n.charAt(i)) + 1);
                else
                    h.put(n.charAt(i) , 1l);
            }

            long num = fact((long)n.length());
            long temp = 1;
            
            for(Character c : h.keySet()) {
                long g = h.get(c);
                temp = ((temp % mod) * (fact(g) % mod)) % mod;
            }

            long ans = ((num % mod) * (modInverse(temp) % mod)) % mod;
            pn(ans);
        }   
    }
