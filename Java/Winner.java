...
public class Winner {
    ...
    public static void main(String[] args)throws IOException {
        ...
        for(int i = 0; i < n; i++) {
            ...
            int sc = f.nextInt();
            isc[i] = sc;
            if(h.get(name) == null) {h.put(name , sc);}
            else h.put(name , h.get(name) + sc);
        }
        //pn(h);
        ...
        for(String i : h.keySet()) { if(h.get(i) == max) cand.add(i);}
        //pn(cand);
        //pn(max);
        String ans = "";
        HashMap<String , Integer> csc = new HashMap<String , Integer>();
        for(int i = 0; i < n; i++) {
            if(cand.contains(iname[i])) {
                if(csc.containsKey(iname[i])) {csc.put(iname[i] , csc.get(iname[i]) + isc[i]);}
                else csc.put(iname[i] , isc[i]);
                if(csc.get(iname[i]) >= max) {ans = iname[i]; break;}
            }
        }
        pn(ans);
        pw.flush();
