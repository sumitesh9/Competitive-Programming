...
public class HackTheMoney {
    ...
    public static boolean onlyten(long n) {
        for(int i = 1; i < 12; i++) {
            if((long)Math.pow(10 , i) == n)
                return true;
        }
        return false;
    }
    
    public static boolean onlytwenty(long n) {
        for(int i = 1; i < 12; i++) {
            if((long)Math.pow(20 , i) == n)
                return true;
        }
        return false;
    }
    ...
    public static boolean solve(long n) {
        ...
        for(int i = 1; i <= 12; i++) {
            for(int j = 1; j <= 12; j++) {
                sum = (long)Math.pow(10 , i) * (long)Math.pow(20 , j);
                if(sum == n)
                    return true;
            }
        }
