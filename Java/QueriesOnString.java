...
public class QueriesOnString {
    ...
    public static String shift(String s , int num) {
        String sub = s.substring(s.length() - num, s.length());
        String res = sub + s.substring(0, s.length() - num);
        return res;
    }
    public static void main(String[] args) {
        ...
        while(m-- > 0) {
            ...
            int r = f.nextInt();
            int k = f.nextInt();
            int num_shift = k % (r - l + 1);
            String res = shift(s.substring(l - 1, r) , num_shift);
            s = s.substring(0, l-1) + res + s.substring(r, s.length());
        }
        pn(s);
    }
