...
public class Subsegments {
    ...
    public static void main(String[] args)throws IOException {
        ...
        int arr[] = inpint(n);
        int l = 0 , r = 0;
        TreeMap<Integer , Integer> tr = new TreeMap<Integer , Integer>(Collections.reverseOrder());
        while(r < n) {
            while((r - l + 1) <= k) {
                if(tr.get(arr[r]) != null) tr.put(arr[r] , tr.get(arr[r]) + 1);
                else tr.put(arr[r] , 1);
                ++r;
            }
            int x = -1;
            for(Integer i : tr.keySet()) {
                if(tr.get(i) == 1) {pn(i); x = i; break;}
            }
            if(x == -1) pn("Nothing");
            if(tr.get(arr[l]) > 1) tr.put(arr[l] , tr.get(arr[l]) - 1); else tr.remove(arr[l]);
            ++l;
        }
        pw.flush();
        pw.close();  
