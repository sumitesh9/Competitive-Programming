...
public class EhabAndPatheticMex {
    ...
    public static void main(String[] args)throws IOException {
        ...
        HashMap<Integer , Integer> deg = new ArrayList<Integer , Integer>();
        for(int i = 0; i < n ; i++) {
            int u = f.nextInt();
            int v = f.nextInt();
            if(deg.containsKey(u))
                ...
                deg.put(u , 1);
            if(deg.containsKey(v))
                deg.put(v , deg.put(v) + 1);
            else
                deg.put(v , 1);
            if(graph.containsKey(u))
                graph.put(u , graph.get(u).add(v));
            else
                graph.put(u , (new ArrayList<Integer>).add(v));
            if(graph.containsKey(v))
                graph.put(v , graph.put(v).add(u));
            else
                graph.put(v , (new ArrayList<Integer>).add(u));
        }
    }
