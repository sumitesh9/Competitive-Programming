...
public class Addrev {
    ...
    public static void main(String[] args) {
        ...
        while(t-- > 0) {
            ...
            String strPattern = "^0+";    
            p = p.trim().replaceAll(strPattern, "");
            if(p.equals(""))
                p = "0";
            q = q.trim().replaceAll(strPattern, "");
            if(q.equals(""))
                q = "0";
            a = Long.parseLong(p);
            b = Long.parseLong(q);
            long temp = a + b;
            String rev = String.valueOf(temp);
            StringBuilder ans = new StringBuilder(String.valueOf(rev));
            ans = ans.reverse();
            String ans1 = ans.toString();
            ans1 = ans1.trim().replaceAll(strPattern, "");
            if(ans1.equals(""))
                ans1 = "0";
            pn(ans1);
        }
