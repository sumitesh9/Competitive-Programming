...
import java.text.*;
public class PointsAndPowerOfTwo {
    ...
    public static void main(String[] args)throws IOException {
        ...
        if(n == 1) {pn(1); p(arr[0]);}
            ...
            for(int i = 0; i < n; i++) {
                long x = arr[i];
                int cs = 1;
                HashSet<Long> ar = new HashSet<Long>();
                ar.add(x);
                for(long j = 1; j <= (long)1e9; j <<= 1) {
                    if(h.contains((1 << j) - x)) {ar.add((1 << j) - x); cs++;}
                }
                pn(ar);
                if(cs > ans) {ans = cs; res.clear(); for(Long j : ar) res.add(j);}
            }
            pn(ans);
            for(Long j : res) {p(j + " ");}
        }
