...
public class PandaAndChainReaction {
    static int mod = (int)Math.pow(10, 6) + 3;
    ...
    public static void main(String[] args) {
        ...
        while(t -- > 0) {
            String n = f.next();
            String x = f.next();
            String mf = "1000000003";
            BigInteger b1 = new BigInteger(n);
            BigInteger b2 = new BigInteger(x);
            BigInteger modb = new BigInteger(mf);
            if(b1.compareTo(modb) >= 0)
                pn(0);
            else {
                int n1 = Integer.parseInt(n);
                pn(((b2.mod(modb)).intValue() * (dp[n1] % mod)) % mod);
            }
        }
    }
