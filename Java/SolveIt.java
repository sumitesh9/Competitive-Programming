...
public class SolveIt {
    ...
    public static double f(int p , int q , int r , int s , int t , int u , double x) {
        return Math.exp(-x) * p + q * Math.sin(x) + r * Math.cos(x) + s * Math.tan(x) + t * x * x + u;
    }
    public static void main(String[] args) {
        while(S.hasNextInt()) {
            ...
            if(f(p , q , r , s , t , u , 0) * f(p , q , r , s , t , u , 1) > 0)
                ...
                for(int i = 0; i < 100; i++) {
                    mid = (l + h) / 2;
                    if(f(p , q , r , s , t , u , 0) * f(p , q , r , s , t , u , mid) <= 0)
                        h = mid;
                    else
                        l = mid;
                }
