...
public class TheyAreEveryWhere {
    ...
    public static void main(String[] args)throws IOException {
        ...
        for(int i = 0; i < n; ++i) {h.add(s.charAt(i));}
        int k = h.size(); h.clear();
        int l = 0 , r = 0;
        int d = -1 , c = 0;
        int resbeg = -1 , resend = -1;
        HashMap<Character , Integer> freq = new HashMap<Character , Integer>();
        while(r < n) {
            if (freq.get(s.charAt(r)) == null) {++c; freq.put(s.charAt(r) , 1);} 
            else freq.put(s.charAt(r) , freq.get(s.charAt(r)) + 1);
            ++r;
            while(l < n && c >= k) {
                if(c == k && (d == -1 || d > (r - l - 1))) {
                    resbeg = l;
                ...
                if (freq.get(s.charAt(l)) == 1) {freq.remove(s.charAt(l)); --c;} 
                else freq.put(s.charAt(l) , freq.get(s.charAt(l)) - 1);
                ++l;
            }
        }
        pn(d + 1);
...
/******************************END OF MAIN PROGRAM*******************************************/
    ...
    static HashSet<Integer> hsi(){return new HashSet<Integer>();}
    static HashSet<Long> hsl(){return new HashSet<Long>();}
    static ArrayList<Integer> ali(){return new ArrayList<Integer>();}
    static ArrayList<Long> all(){return new ArrayList<Long>();}
