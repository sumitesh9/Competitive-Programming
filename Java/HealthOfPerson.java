	...
	public static void main(String[] args) {
		...
		try {
			...
			for (int i = 0; i < cases; i++) {		
				...
				int days[] = new int[m];
				for (int j = 0; j < n; j++) {
					health[j] = Integer.parseInt(x[j]);
				}
				for(int j = 0; j < m; j++) {
					days[j] = Integer.parseInt(y[j]);
				}
				solver.solve(health, days);
			}
		} 
		catch (IOException e) {
