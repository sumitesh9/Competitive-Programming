	...
	public static void main(String[] args) {
		...
		try {
			...
			for (int i = 0; i < cases; i++) {				
				solver.solve(Integer.parseInt(br.readLine()),br.readLine());
			}
 ...
 * 
 */
class Solver extends ChefAndProxy{
	 ...
	 */
	public void solve(int days,String att) {		
               int countp=0;
               for(int i=0;i<days;i++)
               	if(att.charAt(i)=='P')
               		countp++;

               	double r=Math.ceil(0.75*days);
               	int req=(int)r;
               	System.out.println(countp+" "+r+" "+req);
               	if(countp>=req)
               	...
               	else
               	{
               		...
               		{
               			if(att.charAt(i)=='A'&&(att.charAt(i-1)=='P'||att.charAt(i-2)=='P')&&(att.charAt(i+1)=='P'||att.charAt(i+2)=='P'))
               				c++;
               			if(c>=lack)
               				break;
               		}
