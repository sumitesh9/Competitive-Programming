 ...
 * 
 */
class Solver extends CombinationLock{
	/*
	 * Logic goes here ...
	 * Add to the global variables after processing the input
	 * Maybe reverse a string or parse to an integer or , etc.
	 */
	public void solve(String os,String s) {
		 long res=0;
         for(int i=0;i<os.length();i++)
         {
            int d1=Character.getNumericValue(os.charAt(i));
            int d2=Character.getNumericValue(s.charAt(i));
            res+=Math.min(Math.abs(d1-d2),10-Math.abs(d1-d2));
         }
